...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
	{
		for(col_counter = 1; col_counter < table.rows[row_counter].cells.length; col_counter++)
		{
			...
			if(col_counter == 3){
				opp_score = parseInt(cell_value);
			}
		}
		if(home_score > opp_score){
			document.getElementById("stats_table").rows[row_counter].cells[4].innerHTML = "Buffs";
			buff_wins++;
		}
		else{
			document.getElementById("stats_table").rows[row_counter].cells[4].innerHTML = opp_team;
			buff_loss++;
		}
	}
...
/*
	...
	*/
	// text +='<a class = "bootstrap"'
	// text +='<a href="#" onclick = "switchPlayers(0)">'+player[i].name+'<a>';
...
*/
	function switchPlayers(playerNum){
		...
		var games_played = players[playerNum].games_played;
		var passing = players[playerNum].pass_yards;
		var rush = players[playerNum].rushing_yards;
		var rec = players[playerNum].receiving_yards;
		passing = passing/games_played;
		rush = rush/games_played;
		rec = rec/games_played;
		passing = Math.floor(passing);
		rush = Math.floor(rush);
		rec = Math.floor(rec);
